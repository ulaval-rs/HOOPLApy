...
class Oudin(PETModel):
    ...
    def prepare(self, time_step: str, date: datetime, mean_air_temperature: float, latitude: float):
        ...
        ----------
        Re
            Extraterrestrial radiation (MJ/m2/3h)
        lambda_constant
        ...
        day_of_year = find_day_of_year(date)

        self.mean_air_temperature = mean_air_temperature
        self.lambda_constant = 2.501 - 0.002361 * mean_air_temperature  # (MJ / kg)

        # Computation
        Lrad = (np.pi * latitude) / 180  # (rad)
        ds = 0.409 * np.sin((2 * np.pi / 365) * day_of_year - 1.39)  # (rad)
        dr = 1 + 0.033 * np.cos(day_of_year * 2 * np.pi / 365)  # no unit
        omega = np.acos(-np.tan(Lrad) * np.tan(ds))  # (rad)

        if time_step == '3h':
            b = 2. * np.pi * (day_of_year - 81) / 364  # component of the seasonal correction.
            Sc = 0.1645 * np.sin(2 * b) - 0.1255 * np.cos(b) - 0.025 * np.sin(b)  # seasonal correction
            t = date[:, 4] + 0.5  # standard clock time at the midpoint of the period[hour]
