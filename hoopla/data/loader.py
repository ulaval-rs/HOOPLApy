...
from hoopla.data import Observations, validation
from hoopla.pet_models import PETModel
from hoopla.sar_models import SARModel


def load_observations(path: str, file_format: str, config: Config, pet_model: PETModel, sar_model: SARModel) -> dict:
    if file_format == 'mat':
        ...
        observation_dict['dates'] = [datetime(year=d[0], month=d[1], day=d[2], hour=d[3], minute=d[4], second=d[5]) for d in observation_dict.pop('Date')]
