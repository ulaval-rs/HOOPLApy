...
def make_calibration(
    ...
    data_obs = loadmat(
        f"{DATA_PATH}/{config.general.time_step}/Hydromet_obs/Hydromet_obs_{catchment_name}.mat"
    )
    ...
    crop_data(
        config, data_obs, hydro_model, pet_model, sar_model, ini="ini_calibration"
    )
...
def calibrate(
    ...
    model_param_boundaries = loadmat(
        f"{DATA_PATH}/{config.general.time_step}/Model_parameters/model_param_boundaries.mat"
    )
    snow_model_param_boundaries = loadmat(
        f"{DATA_PATH}/{config.general.time_step}/Model_parameters/snow_model_param_boundaries.mat"
    )
    ...
    if config.general.compute_snowmelt:
        ...
        lower_boundaries_of_parameters = model_param_boundaries[hydro_model.name][
            "sMin"
        ][0][0][0]
        upper_boundaries_of_parameters = model_param_boundaries[hydro_model.name][
            "sMax"
        ][0][0][0]
    ...
    if SCORES[config.calibration.score] == NotImplemented:
        raise ValueError(
            f"Score function ({config.calibration.score}) is not implemented"
        )
    ...
    if config.calibration.method == "DDS":
        ...
        raise ValueError(
            f'Calibration method "{config.calibration.method}" not known. '
...
def dynamically_dimensioned_search(
    s_ini: np.array, s_min: np.array, s_max: np.array, max_iter: int
):
