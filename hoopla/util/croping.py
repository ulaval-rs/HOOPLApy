...
def crop_data(
    ...
    dates = np.array(
        [
            datetime(
                year=d[0], month=d[1], day=d[2], hour=d[3], minute=d[4], second=d[5]
            )
            for d in data_obs["Date"]
        ]
    )
    ...
    select = np.isin(
        dates,
        np.arange(date_begin, date_end, timedelta(hours=time_step / 24)).astype(
            datetime
        ),
    )
    ...
    if (
        ...
        or date_begin > max(dates)
        or date_end > max(dates)
        or date_end < min(dates)
    ):
        raise ValueError(
    ...
    if np.sum(np.isnan(data_obs["Q"])) / len(data_obs["Q"]) > 0.75:
        ...
        if ini == "ini_calibration" and np.sum(np.isnan(data_obs["Q"])) == 0:
            raise ValueError(
                "All streamflow are NaN. The calibration is not possible. Please, change calibration dates"
            )
