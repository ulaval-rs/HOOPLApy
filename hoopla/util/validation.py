...
def _potential_evapotranspiration(data_obs: Dict, pet_model: PETModel):
    ...
    if len(parameters) == 0:
        raise ValueError(
            f"PET:Data, data not provided for the {pet_model.name} PET model"
        )
...
def _snow_accounting_validation(data_obs: Dict, sar_model: SARModel):
    ...
    if len(parameters) == 0:
        raise ValueError(
            f"SAR:Data, data not provided for the {sar_model.name} SAR model"
        )
...
def _meteorological_forecast_validation(
    config: Config, data_meteo_forecast: Dict, sar_model: SARModel
):
    if config.forecast.perfect_forecast == 0:
        ...
        if "Pt" not in data_meteo_forecast:
            raise ValueError(
                "Hydrology:Data, Meteorological precipitation forecast not provided."
            )

        if "T" not in data_meteo_forecast:
            raise ValueError(
                "Hydrology:Data, Meteorological mean temperature not provided."
            )

        if "Tmin" not in data_meteo_forecast:
            warnings.warn(
                "Hydrology:Data, Tmin meteorological forecast not provided. Tmin set to NaN."
            )
            ...
            if config.general.compute_snowmelt and "CemaNeige" == sar_model.name:
                warnings.warn(
                    "Hydrology:Data, because the meteorological forecast for Tmin is missing, "
        ...
        if "Tmax" not in data_meteo_forecast:
            warnings.warn(
                "Hydrology:Data, Tmax meteorological forecast not provided. Tmin set to NaN."
            )
            ...
            if config.general.compute_snowmelt and "CemaNeige" == sar_model.name:
                warnings.warn(
                    "Hydrology:Data, because the meteorological forecast for Tmax is missing, "
                    "the USGS function is used to compute snow fraction. "
                    "This may result in a decrease of performance, especially if the "
                    "Hydrodel function was used during calibration"
                )
